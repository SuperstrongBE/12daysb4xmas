...
const getClaimable = cache(async (): Promise<TimedTemplate | null | undefined> => {
  ...
  return timedTemplates.find((template)=>isActiveRightNow(template.activeStartTime,template.activeEndTime) );
  ...
  return (
    <>
      <main className="grid grid-cols-1 gap-8 md:pb-8 mx-8" > 
        ...
        {data ? (
          <NFTLayout template={data as ITemplate} ipfsResolver={process.env.NEXT_PUBLIC_IPFS_ENDPOINT!}></NFTLayout>
        ) : (
          <p className="text-center">Nothing to mint</p>
        )}
